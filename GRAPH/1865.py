...
### 수도코드 ###
# 테스트케이스 개수 T 입력
# N(지점 수), M(도로 개수), W(웜홀 개수) 입력
# M만큼 도로 정보 입력 (S, E, T) -> 양방향이므로 (E, S, T)도 저장
# W만큼 웜홀 정보 입력 (S, E, T)
# graphs = []
# N-1만큼 모든 간선에 대해서 최소값 업데이트 (더 작아진 경우만 업데이트)
# N에서도 업데이트가 발생하면 -> YES (음수 사이클 존재)
...
for _ in range(T):
    ...
    graphs += [(0, i, 0) for i in range(1, N+1)]
    # print(f"graphs: {graphs}")
    for i in range(N+1):
        if cycle_found: break
        for (u, v, cost) in graphs:
