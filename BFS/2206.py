# N×M의 행렬로 표현되는 맵이 있다. 맵에서 0은 이동할 수 있는 곳을 나타내고, 1은 이동할 수 없는 벽이 있는 곳을 나타낸다. 
# 당신은 (1, 1)에서 (N, M)의 위치까지 이동하려 하는데, 이때 최단 경로로 이동하려 한다. 최단경로는 맵에서 가장 적은 개수의 칸을 지나는 경로를 말하는데, 이때 시작하는 칸과 끝나는 칸도 포함해서 센다.

# 만약에 이동하는 도중에 한 개의 벽을 부수고 이동하는 것이 좀 더 경로가 짧아진다면, 벽을 한 개 까지 부수고 이동하여도 된다.

# 한 칸에서 이동할 수 있는 칸은 상하좌우로 인접한 칸이다.

# 맵이 주어졌을 때, 최단 경로를 구해 내는 프로그램을 작성하시오.

# 첫째 줄에 N(1 ≤ N ≤ 1,000), M(1 ≤ M ≤ 1,000)이 주어진다. 
# 다음 N개의 줄에 M개의 숫자로 맵이 주어진다. (1, 1)과 (N, M)은 항상 0이라고 가정하자.

# 첫째 줄에 최단 거리를 출력한다. 불가능할 때는 -1을 출력한다.

### 수도코드 ###
# 큐 - (x, y, wall, distance)

from collections import deque

N, M = map(int, input().split())
MAP = [list(map(int, input())) for _ in range(N)]
MIN = float('inf')
changed = False
visited = [[[False for _ in range(M)] for _ in range(N)] for _ in range(2)]
# print(f"lst_map: {MAP}")
# print(f"visited: {visited}")

queue = deque()
queue.append((0, 0, 1, 1))
visited[1][0][0] = True

dx = [-1, 0, 1, 0]
dy = [0, -1, 0, 1]

while queue:
    x, y, w, dist = queue.popleft()
    # print(f"x: {x}, y: {y}, w: {w}, dist: {dist}")

    if x == N-1 and y == M-1:
        MIN = min(MIN, dist)
        changed = True
        # print(f"MIN: {MIN}")

    for i in range(4):
        nx = x + dx[i]
        ny = y + dy[i]
        if 0<=nx<N and 0<=ny<M:
            if MAP[nx][ny]==0 and not visited[w][nx][ny]:
                visited[w][nx][ny] = True
                queue.append((nx, ny, w, dist+1))
                # print(f"queue: {queue}")

            if MAP[nx][ny]==1:
                if w == 1 and not visited[w-1][nx][ny]:
                    visited[w-1][nx][ny] = True
                    queue.append((nx, ny, w-1, dist+1))
                    # print(f"queue: {queue}")

if changed:
    print(MIN)
else:
    print(-1)

### 테스트케이스 1 ###
# 6 4
# 0100
# 1110
# 1000
# 0000
# 0111
# 0000

# 15

### 테스트케이스 2 ###
# 4 4
# 0111
# 1111
# 1111
# 1110

# -1